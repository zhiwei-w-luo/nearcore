...
// #[wasm_bindgen]
// pub fn pass_map(map: JsValue) -> JsValue {
//     let mut map: Result<HashMap<u64, Vec<u8>>, Error> = serde_wasm_bindgen::from_value(map);
//     if map.is_err() {
//         console::log_2(&"{}".into(), &map.err().unwrap().into());
//         unreachable!()
//     } else {
//         let mut map2 = map.ok().unwrap();
//         let i: u64 = 0;
//         serde_wasm_bindgen::to_value(&map2).unwrap()
//     }
// }

// #[wasm_bindgen]
// pub fn pass_context(context: JsValue) {
//     use web_sys::console;
//     console::log_1(&"hello".into());
//     let mut map: Result<VMContext, Error> = serde_wasm_bindgen::from_value(context);
//     if map.is_err() {
//         console::log_1(&map.err().unwrap().into());
//         unreachable!()
//     } else {
//     }
// }
...
impl VM {
    ...
    //     self.state.registers = registers.clone();
    // }

    fn runVM<T, F: FnOnce(&mut VMLogic) -> VMResult<T>>(&mut self, f: F, register_id: Option<u64>) -> T {
    ...
    // fn update_a_register(register_id: u64, logic: &mut VMLogic, state: &mut SimpleState, memory: &mut MockedMemory) {
    ...
    pub fn read_register(&mut self, register_id: u64, ptr: u64) {
        ...
        self.runVM(|vm| vm.read_register(register_id, ptr), None)
    }
    ...
    pub fn register_len(&mut self, register_id: u64) -> u64 {
        self.runVM(|vm| vm.register_len(register_id), None)
    }
    ...
    pub fn write_register(&mut self, register_id: u64, data_len: u64, data_ptr: u64) -> () {
        let res = self.runVM(|vm| {
            vm.write_register(register_id, data_len, data_ptr)
        },
        Some(register_id));
        res
    }
    ...
    pub fn current_account_id(&mut self, register_id: u64) -> () {
        self.runVM(|vm| vm.current_account_id(register_id), Some(register_id))
    }
    ...
    pub fn signer_account_id(&mut self, register_id: u64) -> () {
        self.runVM(|vm| vm.signer_account_id(register_id), Some(register_id))
    }
    ...
    pub fn signer_account_pk(&mut self, register_id: u64) -> () {
        self.runVM(|vm| vm.signer_account_pk(register_id), Some(register_id))
    }
    ...
    pub fn predecessor_account_id(&mut self, register_id: u64) -> () {
        self.runVM(|vm| vm.predecessor_account_id(register_id), Some(register_id))
    }
    ...
    pub fn input(&mut self, register_id: u64) -> () {
        self.runVM(|vm| vm.input(register_id), Some(register_id))
    }
    ...
    pub fn block_index(&mut self) -> u64 {
        self.runVM(|vm| vm.block_index(), None)
    }
    ...
    pub fn block_timestamp(&mut self) -> u64 {
        self.runVM(|vm| vm.block_timestamp(), None)
    }
    ...
    pub fn storage_usage(&mut self) -> StorageUsage {
        self.runVM(|vm| vm.storage_usage(), None)
    }
    // // #################
    // // # Economics API #
    // // #################
    ...
    pub fn account_balance(&mut self, balance_ptr: u64) -> () {
        self.runVM(|vm| vm.account_balance(balance_ptr), None)
    }
    ...
    pub fn account_locked_balance(&mut self, balance_ptr: u64) -> () {
        self.runVM(|vm| vm.account_locked_balance(balance_ptr), None)
    }
    ...
    pub fn attached_deposit(&mut self, balance_ptr: u64) -> () {
        self.runVM(|vm| vm.attached_deposit(balance_ptr), None)
    }
    ...
    pub fn prepaid_gas(&mut self) -> Gas {
        self.runVM(|vm| vm.prepaid_gas(), None)
    }
    ...
    pub fn used_gas(&mut self) -> Gas {
        self.runVM(|vm| vm.used_gas(), None)
    }
    // // ############
    // // # Math API #
    // // ############
    ...
    pub fn random_seed(&mut self, register_id: u64) -> () {
        self.runVM(|vm| vm.random_seed(register_id), Some(register_id))
    }
    ...
    pub fn sha256(&mut self, value_len: u64, value_ptr: u64, register_id: u64) -> () {
        self.runVM(|vm| vm.sha256(value_len, value_ptr, register_id), Some(register_id))
    ...
    pub fn gas(&mut self, gas_amount: u32) -> () {
        self.runVM(|vm| vm.gas(gas_amount), None)
    }
    ...
    //     gas: Gas,
    // ) -> u64 {
    //     self.runVM(|vm| vm.promise_create(.unwrap())
    //         account_id_len,
    //         account_id_ptr,
    //         method_name_len,
    //         method_name_ptr,
    ...
    //     gas: u64,
    // ) -> u64 {
    //     self.runVM(|vm| vm.promise_then(.unwrap())
    //         promise_idx,
    //         accound_id_len,
    //         account_id_ptr,
    //         method_name_len,
    ...
    // ) -> PromiseIndex {
    //     self.runVM(|vm| vm.promise_and(promise_ids_ptr, promise_idx_count).unwrap())
    // }
    ...
    //     account_id_ptr: u64,
    // ) -> u64 {
    //     self.runVM(|vm| vm.promise_batch_create(accound_id_len, account_id_ptr).unwrap())
    // }
    ...
    //     account_id_ptr: u64,
    // ) -> u64 {
    //     self.runVM(|vm| vm.promise_batch_then(promise_idx, accound_id_len, account_id_ptr).unwrap())
    // }
    ...
    //     self.runVM(|vm| vm.promise_batch_action_create_account(promise_idx).unwrap())
    // }
    ...
    //     code_ptr: u64,
    // ) -> () {
    //     self.runVM(|vm| vm.promise_batch_action_deploy_contract(promise_idx, code_len, code_ptr).unwrap())
    // }
    ...
    //     gas: Gas,
    // ) -> () {
    //     self.runVM(|vm| vm.promise_batch_action_function_call(promise_idx, method_name_len, method_name_ptr, arguments_len, arguments_ptr, amount_ptr, gas).unwrap())
    // }
    ...
    //     amount_ptr: u64,
    // ) -> () {
    //     self.runVM(|vm| vm.promise_batch_action_transfer(promise_idx, amount_ptr).unwrap())
    // }
    ...
    //     public_key_ptr: u64,
    // ) -> () {
    //     self.runVM(|vm| vm.promise_batch_action_stake(promise_idx, amount_ptr, public_key_len, public_key_ptr).unwrap())
    // }
    ...
    //     nonce: u64,
    // ) -> () {
    //     self.runVM(|vm| vm.promise_batch_action_add_key_with_full_access(promise_idx, public_key_len, public_key_ptr, nonce).unwrap())
    // }
    ...
    //     public_key_ptr: u64,
    // ) -> () {
    //     self.runVM(|vm| vm.promise_batch_action_delete_key(promise_idx, public_key_len, public_key_ptr).unwrap())
    // }
    ...
    // ) -> () {
    //    self.runVM(|vm| vm.promise_batch_action_delete_account(promise_idx, beneficiary_id_len, beneficiary_id_ptr) .unwrap())
    // }
    ...
    //     self.runVM(|vm| vm.promise_results_count().unwrap())
    // }
    // // If the current function is invoked by a callback we can access the execution results of the
    ...
    //     self.runVM(|vm| vm.promise_result(result_idx, register_id).unwrap())
    // }
    ...
    //     self.runVM(|vm| vm.promise_return(promise_idx).unwrap())
    // }
    // // #####################
    // // # Miscellaneous API #
    // // #####################
    ...
    pub fn value_return(&mut self, value_len: u64, value_ptr: u64) -> () {
        self.runVM(|vm| vm.value_return(value_len, value_ptr), None)
    }
    ...
    pub fn panic_utf8(&mut self, len: u64, ptr: u64) -> () {
        self.runVM(|vm| vm.panic_utf8(len, ptr), None)
    }
    ...
    pub fn log_utf8(&mut self, len: u64, ptr: u64) -> () {
        self.runVM(|vm| vm.log_utf8(len, ptr), None)
    }
    ...
    pub fn log_utf16(&mut self, len: u64, ptr: u64) -> () {
        self.runVM(|vm| vm.log_utf16(len, ptr), None)
    }
    ...
    pub fn abort(&mut self, msg_ptr: u32, filename_ptr: u32, line: u32, col: u32) -> () {
        self.runVM(|vm| vm.abort(msg_ptr, filename_ptr, line, col), None)
    }
    // // ###############
    // // # Storage API #
    // // ###############
    ...
    pub fn storage_write(
        ...
        self.runVM(|vm| vm.storage_write(key_len, key_ptr, value_len, value_ptr, register_id), Some(register_id))
    }
    ...
    pub fn storage_read(&mut self, key_len: u64, key_ptr: u64, register_id: u64) -> u64 {
        self.runVM(|vm| vm.storage_read(key_len, key_ptr, register_id), Some(register_id))
    }
    ...
    pub fn storage_remove(&mut self, key_len: u64, key_ptr: u64, register_id: u64) -> u64 {
        self.runVM(|vm| vm.storage_remove(key_len, key_ptr, register_id), Some(register_id))
    }
    ...
    pub fn storage_has_key(&mut self, key_len: u64, key_ptr: u64) -> u64 {
        self.runVM(|vm| vm.storage_has_key(key_len, key_ptr), None)
    }
    ...
    pub fn storage_iter_prefix(&mut self, prefix_len: u64, prefix_ptr: u64) -> u64 {
        self.runVM(|vm| vm.storage_iter_prefix(prefix_len, prefix_ptr), None)
    }
    ...
    pub fn storage_iter_range(
        ...
        self.runVM(|vm| vm.storage_iter_range(start_len, start_ptr, end_len, end_ptr), None)
    }
    ...
    pub fn storage_iter_next(
        ...
        self.runVM(|vm| vm.storage_iter_next(iterator_id, key_register_id, value_register_id), None)
    }

    // // Computes the outcome of execution.
    // pub fn outcome(self) -> VMOutcome {
    //     self.runVM(|vm| vm.outcome().unwrap())
    // }
}
