...
impl TrieNodeWithSize {
    ...
    fn from_node(node: TrieNode, storage: &NodesStorage) -> TrieNodeWithSize {
        let memory_usage = 0; //TODO
...
/// Raw trie node. hash(encode(raw trie node)) = what is used in cryptographic proofs
enum RawTrieNode {
    Leaf(Vec<u8>, Vec<u8>),
...
/// memory_usage is serialized and stored, but does not contribute to hash
#[derive(Debug, Eq, PartialEq)]
...
impl RcTrieNode {
    ...
    fn decode(bytes: &[u8]) -> Result<(RawTrieNodeWithSize, u32), std::io::Error> {
        let node = RawTrieNodeWithSize::decode(&bytes[..bytes.len() - 4])?;
        let mut cursor = Cursor::new(&bytes[bytes.len() - 4..]);
        let rc = cursor.read_u32::<LittleEndian>()?;
        Ok((node, rc))
    }
...
impl TrieCachingStorage {
    ...
    fn retrieve_node(&self, hash: &CryptoHash) -> Result<TrieNodeWithSize, String> {
        if let Some(bytes) = self.retrieve_raw_bytes(hash) {
            match RcTrieNode::decode(&bytes) {
                Ok((value, _)) => Ok(TrieNodeWithSize::from_raw(value)),
...
impl Trie {
    ...
    fn flatten_nodes(
        ...
        while let Some((node, position)) = stack.pop() {
            ...
            eprintln!("hehe node with size {}", memory_usage);
...
impl<'a> Iterator for TrieIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        enum IterStep {
            Continue,
            PopTrail,
            Descend(Result<Box<TrieNodeWithSize>, String>),
        }
        loop {
            let iter_step = {
...
mod tests {
    ...
    fn gen_changes(rng: &mut impl Rng) -> Vec<(Vec<u8>, Option<Vec<u8>>)> {
        ...
        //        let delete_probability = 0.0; //TODO
